...
public class SantanderCardsRequestTask extends CronTask {
    ...
    public boolean canRequestFirstCard(User user) {
        ...
        //If SantanderEntry has WAITING_INFO and "WAITING_DAYS" have passed, send automatic request
        ...
        //If user has no SantanderEntry, and a valid role (different than STUDENT) send a notification to check the card information
        try {
            SantanderUser santanderUser = new SantanderUser(user, userInfoService);
            String userRole = santanderUser.getRole();
            if (user.getCurrentSantanderEntry() == null && !userRole.equals("STUDENT")) {
                return true;
            }
            ...
            logger.error(
                    String.format("Failed checking user %s role (current SantanderEntry: %s)", user.getUsername(),
